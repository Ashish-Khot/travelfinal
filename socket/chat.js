...
function setupSocket(server) {
  ...
  io.on('connection', (socket) => {
    ...
    /**
     * ITINERARY REAL-TIME COLLABORATION
     * Events for collaborative itinerary editing
     */

    // Join itinerary collaboration room
    socket.on('joinItineraryRoom', ({ itineraryId, userId }) => {
      socket.join(`itinerary_${itineraryId}`);
      console.log(`User ${userId} joined itinerary room: ${itineraryId}`);
      // Notify others in the room
      socket.to(`itinerary_${itineraryId}`).emit('userJoinedItinerary', {
        userId,
        timestamp: new Date(),
      });
    });

    // Leave itinerary collaboration room
    socket.on('leaveItineraryRoom', ({ itineraryId, userId }) => {
      socket.leave(`itinerary_${itineraryId}`);
      console.log(`User ${userId} left itinerary room: ${itineraryId}`);
      socket.to(`itinerary_${itineraryId}`).emit('userLeftItinerary', {
        userId,
        timestamp: new Date(),
      });
    });
    ...
    socket.on('activityAdded', ({ itineraryId, activity }) => {
      socket.to(`itinerary_${itineraryId}`).emit('activityAdded', {
        itineraryId,
        activity,
        timestamp: new Date(),
      });
    ...
    // Activity updated
    socket.on('activityUpdated', ({ itineraryId, activityId, updates }) => {
      socket.to(`itinerary_${itineraryId}`).emit('activityUpdated', {
        itineraryId,
        activityId,
        updates,
        timestamp: new Date(),
    ...
    socket.on('activityRemoved', ({ itineraryId, activityId }) => {
      socket.to(`itinerary_${itineraryId}`).emit('activityRemoved', {
        itineraryId,
        activityId,
        timestamp: new Date(),
      });
    ...
    // Itinerary updated (title, dates, etc.)
    socket.on('itineraryUpdated', ({ itineraryId, updates }) => {
      socket.to(`itinerary_${itineraryId}`).emit('itineraryUpdated', {
        itineraryId,
        updates,
        timestamp: new Date(),
      });
    ...
    socket.on('activitiesReordered', ({ itineraryId, newOrder }) => {
      socket.to(`itinerary_${itineraryId}`).emit('activitiesReordered', {
        itineraryId,
        newOrder,
        timestamp: new Date(),
      });
    ...
    // Budget updated
    socket.on('budgetUpdated', ({ itineraryId, budget }) => {
      socket.to(`itinerary_${itineraryId}`).emit('budgetUpdated', {
        itineraryId,
        budget,
        timestamp: new Date(),
      });
    ...
    // Typing activity indicator for itinerary
    socket.on('typingInItinerary', ({ itineraryId, userId }) => {
      socket.to(`itinerary_${itineraryId}`).emit('userTypingInItinerary', {
        userId,
        timestamp: new Date(),
      });
    });

    // Stop typing in itinerary
    socket.on('stoppedTypingInItinerary', ({ itineraryId, userId }) => {
      socket.to(`itinerary_${itineraryId}`).emit('userStoppedTypingInItinerary', {
        userId,
        timestamp: new Date(),
      });
    });

    // Comment added to itinerary
    socket.on('commentAdded', ({ itineraryId, comment }) => {
      socket.to(`itinerary_${itineraryId}`).emit('commentAdded', {
        itineraryId,
        comment,
        timestamp: new Date(),
      });
